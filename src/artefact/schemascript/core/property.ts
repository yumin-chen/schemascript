...
> {
	...
	toString(): string {
		...
		const defaultVal = this.hasDefault
			? `.default(${typeof this.defaultValue === "bigint" ? this.defaultValue.toString() : this.defaultValue && typeof this.defaultValue === "object" && "queryChunks" in (this.defaultValue as Record<string, unknown>) ? "sql`...`" : JSON.stringify(this.defaultValue)})`
			: "";
