...
`;
        ...
        const result = await runMigrationTest("schema_e2e_optional", schemaContent, fallbackSchema);
    ...
    test("generated SQL should correctly reflect the optional column", () => {
		...
		expect(sqlContent.toLowerCase()).not.toContain('optional_col` text not null');
        expect(sqlContent.toLowerCase()).not.toContain('"optional_col" text not null');
    });
});
